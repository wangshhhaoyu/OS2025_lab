...
void proc_run(struct proc_struct *proc)
{
    ...
    bool intr_flag;
        ...
        local_intr_save(intr_flag); // 关中断，保证切换的原子性
        {
            current = proc;               // 更新当前进程
            // 在上下文切换前切换地址空间到目标进程的页表
            lsatp(next->pgdir);
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        // LAB5: use set_links to set the relation links of process
        set_links(proc);
    }
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    *(int *)stacktop = argc;
    ...
    // 保留 sstatus 中的某些位（如 SPP），并清除其他位，设置用户模式
    uintptr_t sstatus = read_csr(sstatus);
    sstatus &= ~SSTATUS_SPP;  // 清除 SPP 位，表示返回用户态
    sstatus |= SSTATUS_SPIE;  // 设置 SPIE，返回用户态后开中断
    sstatus &= ~SSTATUS_SIE;  // 清除 SIE
