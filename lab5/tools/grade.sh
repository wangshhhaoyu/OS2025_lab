...
# 在部分 QEMU/OpenSBI 版本组合中，使用 -device loader 只会把镜像放入内存，但不会设置 OpenSBI 的 Next Address，
# 导致 QEMU 启动后停在 OpenSBI，串口文件为空（no $qemu_out）。改为 -kernel 方式，OpenSBI 会把 payload
# 设为下一跳地址并传递 DTB 指针，从而内核正常起跑并有串口输出。
